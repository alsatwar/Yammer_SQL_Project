...
SELECT experiment_group,
       ...
       ROUND((Avg_logins - Avg_Control_Logins)/SQRT(Var/Users),4) AS 't1_stat',
       (1 - COALESCE(nd.value,1))*2 AS p_value
FROM table3
LEFT JOIN yammer_project.z_p_values nd
ON nd.score =ABS(ROUND((Avg_logins - Avg_Control_Logins)/SQRT((Var/Users)+(Control_Variance/Control_Users)),3)) 